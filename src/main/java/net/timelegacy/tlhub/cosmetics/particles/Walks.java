    ...
    Bukkit.getScheduler().scheduleSyncRepeatingTask(plugin, () -> {
      for (Player p : Bukkit.getOnlinePlayers()) {
        if (CosmeticHandler.particleEnabled(p, "SNOWWALK")) {
          ParticleUtils.display(Particle.CLOUD, p.getEyeLocation().add(0, -1.5, 0), 10, 0);
        }

        if (CosmeticHandler.particleEnabled(p, "FIREWALK")) {
          ParticleUtils.display(Particle.FLAME, p.getEyeLocation().add(0, -1.5, 0), 100, 0);
        }

        if (CosmeticHandler.particleEnabled(p, "PORTALWALK")) {
          ParticleUtils.display(Particle.PORTAL, p.getEyeLocation().add(0, -1.5, 0), 1000, 0);
        }

        if (CosmeticHandler.particleEnabled(p, "CRITICALWALK")) {
          ParticleUtils.display(Particle.CRIT, p.getEyeLocation().add(0, -1.5, 0), 100, 0);
        }

        if (CosmeticHandler.particleEnabled(p, "MAGICWALK")) {
          ParticleUtils.display(Particle.SPELL_WITCH, p.getEyeLocation().add(0, -1.5, 0), 100, 0);
        }
      }
